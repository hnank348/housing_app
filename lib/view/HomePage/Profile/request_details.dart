              ...
              children: [
                ...
                Button(text: 'Approved'.tr(), color:const Color(0xff2D5C7A) , colorText: Colors.white,
                    onPressed: ()async{
                      ...
                      if (confirm == true) {
                        String result = await ApprovedRequests(booking.bookingId,booking.client.id);
                ...
                Button(text: 'Reject'.tr(), color:const Color(0xff2D5C7A) , colorText: Colors.white,
                  onPressed: ()async{
                    ...
                    if (confirm == true) {
                      String result = await RejectRequests(booking.bookingId,booking.client.id);
                      if (context.mounted) showMessage(context, result);
                    }
                  },),
                SizedBox(height: 20,),
                Button(text: 'Delete'.tr(), color:const Color(0xff2D5C7A) , colorText: Colors.white,
                  onPressed: ()async{
                    ...
                    if (confirm == true) {
                      String result = await DeleteRequests(booking.bookingId,booking.client.id);
                      if (context.mounted) showMessage(context, result);
                    }
                  },),
              ],
